...
2. **Коммит и пуш** — агент выполняет сам из **корня репозитория** (workspace path, например `cd /var/home/user/Projects/voiceforge`): `git add` затронутые файлы, осмысленный `git commit`, затем `git push`. Не предлагать команды из домашней директории — иначе «not a git repository».
