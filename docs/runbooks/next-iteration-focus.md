...
**Обновлено:** 2026-03-08 (closed #116; следующий batch = #114 `main.py` / `server_async.py`)
...
- **Новая стратегическая очередь #114-#123:** ручных действий для её сопровождения не требуется; после закрытия `#115` и `#116` текущий practical execution order на ближайшие сессии: **#114 -> #117 -> #118**.
...
**Сделано в сессии:** закрыт **#116**. Добавлен отдельный suite [tests/test_daemon_batch116.py](../../tests/test_daemon_batch116.py) с behavioral/smoke coverage для `src/voiceforge/core/daemon.py`: `analyze`, `listen_start/stop`, D-Bus emitter loop, session/transcript/search/calendar helpers, streaming hooks/loop, retention helper, calendar autostart helper. Локальная targeted coverage-проверка для `voiceforge.core.daemon` на daemon/dbus subset даёт **75%** против baseline `38%`; `pyproject.toml` не менялся, потому что `daemon.py` уже был в coverage report и batch не требовал честного снятия из `omit`. Обновлён [PROJECT-STATUS-SUMMARY.md](PROJECT-STATUS-SUMMARY.md): `#116` отмечен как закрытый, practical queue сдвинут на `#114 -> #117 -> #118`.

**Следующий шаг:** брать **#114** как отдельный coherent batch по hotspot decomposition в `src/voiceforge/main.py` и/или `src/voiceforge/web/server_async.py`. Цель: 1-2 extraction batches с targeted tests, уменьшая локальную сложность без cross-cutting rewrite. Не смешивать этот batch с daemon/RAG/release/docs.
...
- **Открытые issues:** #65 (CVE — ждём upstream) и стратегическая очередь **#114**, **#117-#123**. `#115` и `#116` уже закрыты; следующий code-heavy кандидат — `#114`, затем `#117`, затем `#118`. #50 (macOS/WSL2) закрыт 2026-03-07 — снят с активного скоупа.
...
Задача: взять верхний coherent batch из блока «Следующий шаг» и GitHub Project. На сейчас это **#114**: hotspot decomposition в `src/voiceforge/main.py` и/или `src/voiceforge/web/server_async.py`. Сделай 1-2 extraction batches с targeted tests и без смешивания с daemon/RAG/release surfaces.
```
...
Задача: выполнить следующий coherent batch из блока «Следующий шаг» и GitHub Project, сохраняя batching discipline: один subsystem, один verification loop, один честный handoff. На сейчас это **#114** по `main.py` / `server_async.py`; `#117` и `#118` трогать только в следующих сессиях.
```
...
|----------|--------|----------|
| **P0** | #114 | Hotspot decomposition для `main.py`, `server_async.py` |
