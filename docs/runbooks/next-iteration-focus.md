...
**Обновлено:** 2026-03-08 (closed #121; следующий batch = #122 release proof)
...
- **Новая стратегическая очередь #114-#123:** ручных действий для её сопровождения не требуется; после закрытия `#114`, `#115`, `#116`, `#117`, `#118`, `#119`, `#120` и `#121` текущий practical execution order на ближайшие сессии: **#122 -> #123**.
...
**Сделано в сессии:** закрыт **#121**. В [observability-alerts.md](observability-alerts.md) собран единый reproducible Jaeger/runtime proof path: preflight через `uv run pytest tests/test_otel.py tests/test_observability.py -q --tb=line`, topology guidance для host/toolbox, exact expected spans (`pipeline.run`, `pipeline.prepare_audio`, `pipeline.step1_stt`, `pipeline.step2_parallel`), expected evidence artifacts, failure signatures и cleanup. В [PROJECT-STATUS-SUMMARY.md](PROJECT-STATUS-SUMMARY.md) синхронизирована честная manual boundary: live Jaeger container, runtime trigger и UI verification по-прежнему выполняются пользователем, а не агентом; batch закрыт как documented proof path, а не как automated GUI capture.

**Следующий шаг:** брать **#122** как отдельный coherent batch только по release proof beyond metadata contract. Цель: собрать reproducible path для `cargo-audit`, native desktop gate и updater readiness/disabled state, обновить release docs/summary по фактической manual boundary и не смешивать этот batch с observability, security или broad docs sweep.
...
- **Открытые issues:** #65 (CVE — ждём upstream) и стратегическая очередь **#122-#123**. `#114`, `#115`, `#116`, `#117`, `#118`, `#119`, `#120` и `#121` уже закрыты; следующий candidate — `#122`, затем `#123`. #50 (macOS/WSL2) закрыт 2026-03-07 — снят с активного скоупа.
...
Задача: взять верхний coherent batch из блока «Следующий шаг» и GitHub Project. На сейчас это **#122**: release proof beyond metadata contract. Собери reproducible path для `cargo-audit`, native desktop gate и updater readiness/disabled state и удержи release docs/summary строго по фактической manual boundary.
```
...
Задача: выполнить следующий coherent batch из блока «Следующий шаг» и GitHub Project, сохраняя batching discipline: один subsystem, один verification loop, один честный handoff. На сейчас это **#122** по release proof; `#123` трогать только в следующей сессии.
```
...
| **P1 evidence/manual** | #122, #123 | Release proof, docs/governance sweep |
