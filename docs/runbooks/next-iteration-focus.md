...
**Обновлено:** 2026-03-07 (автопилот: блоки 37, 43, 50, 54, 59, 76 roadmap-100)
...
**Сделано в сессии:** Реализованы блоки 37, 43, 50, 54, 59, 76 из roadmap-100-blocks: панель быстрых действий (Запись, Анализ 60 сек), копирование одного сегмента в деталях сессии, офлайн-баннер при отключении демона с кнопкой «Повторить подключение», `daemon --foreground` (логи в stdout), документация VOICEFORGE_* в config-env-contract.md (индекс + VOICEFORGE_WEB_ASYNC), статистика RAG в status/doctor (indexed_sources_count, chunks_count). Roadmap обновлён.

**Следующий шаг:** Продолжить roadmap-100: блоки 31 (настраиваемые горячие клавиши), 32 (мини-карта сессии), 47 (поиск по action items), 52 (status --json), 58 (calendar list) или ручная проверка десктопа.
