...
**Обновлено:** 2026-03-08 (closed #114; следующий batch = #117 `rag/*`)
...
- **Новая стратегическая очередь #114-#123:** ручных действий для её сопровождения не требуется; после закрытия `#114`, `#115` и `#116` текущий practical execution order на ближайшие сессии: **#117 -> #118 -> #119**.
...
**Сделано в сессии:** закрыт **#114**. В [server_async.py](../../src/voiceforge/web/server_async.py) вынесен общий async web glue для response/request handling (`_response_from_sync_result`, `_to_thread_response`, `_json_request_to_response`, `_invalid_json_body`), что уменьшило повторение в `_build_app` без изменения контракта. В [main.py](../../src/voiceforge/main.py) вынесены общие CLI emit helpers (`_emit_success`, `_calendar_poll_emit`, `_calendar_create_emit`) и через них упрощены `status`, `calendar poll`, `calendar create-from-session`. Добавлен focused suite [tests/test_hotspot_batch114.py](../../tests/test_hotspot_batch114.py), а также перепроверен существующий web/CLI subset; контракт не сдвинулся. Обновлён [PROJECT-STATUS-SUMMARY.md](PROJECT-STATUS-SUMMARY.md): `#114` отмечен как закрытый, practical queue сдвинут на `#117 -> #118 -> #119`.

**Следующий шаг:** брать **#117** как отдельный coherent batch только по heavy RAG confidence в `src/voiceforge/rag/*`. Цель: reproducible smoke/test path для index/search/restore lifecycle и явная граница того, что остаётся ONNX-heavy/manual. Не смешивать этот batch с main/server/daemon/release/docs.
...
- **Открытые issues:** #65 (CVE — ждём upstream) и стратегическая очередь **#117-#123**. `#114`, `#115` и `#116` уже закрыты; следующий code-heavy кандидат — `#117`, затем `#118`, затем `#119`. #50 (macOS/WSL2) закрыт 2026-03-07 — снят с активного скоупа.
...
Задача: взять верхний coherent batch из блока «Следующий шаг» и GitHub Project. На сейчас это **#117**: heavy RAG confidence в `src/voiceforge/rag/*`. Сделай reproducible smoke/test path для index/search/restore lifecycle и отдельно зафиксируй, что остаётся ONNX-heavy/manual.
```
...
Задача: выполнить следующий coherent batch из блока «Следующий шаг» и GitHub Project, сохраняя batching discipline: один subsystem, один verification loop, один честный handoff. На сейчас это **#117** по heavy RAG confidence; `#118` и `#119` трогать только в следующих сессиях.
```
