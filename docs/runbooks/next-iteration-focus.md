...
**Обновлено:** 2026-03-07 (автопилот: i18n плейсхолдеры и кнопка записи)
...
**Сделано в сессии:** i18n (блок 97): поддержка data-i18n-placeholder в applyUiLang; плейсхолдеры поиска сессий (по ID/дате, по тексту транскрипта); кнопка «Старт/Стоп записи» и подпись «Запись идёт» через t() в setListenButtonState и applyUiLang; обновлён roadmap.
