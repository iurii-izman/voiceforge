...
**Обновлено:** 2026-03-07 (автопилот: блоки 31, 32, 47, 52, 58 roadmap-100)
...
**Сделано в сессии:** Реализованы блоки 31, 32, 47, 52, 58 из roadmap-100: настраиваемые горячие клавиши (поля Запись/Анализ в настройках, Store), мини-карта сессии (кнопки по сегментам, scroll to segment), фильтр «Только с action items» (get_session_ids_with_action_items в TranscriptLog/D-Bus/Tauri), status --output json подтверждён, calendar list (list_calendars, voiceforge calendar list). Roadmap обновлён.

**Следующий шаг:** Продолжить roadmap-100: блоки 33 (подсветка поиска в транскрипте), 34 (уведомления по анализу), 38 (теги сессий), 53 (единый --output), 62 (SessionCreated) или ручная проверка десктопа.
