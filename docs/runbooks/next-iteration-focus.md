...
**Обновлено:** 2026-03-07 (автопилот: блоки 33, 34, 38, 53 roadmap-100)
...
**Сделано в сессии:** Реализованы блоки 33, 34, 38, 53: подсветка FTS-запроса в транскрипте при открытии из поиска (highlightQuery, highlightSegmentText), уведомления по анализу (уже были), теги сессий (Store, фильтр по тегу, поле тегов в деталях), единый --output (документация в cli-commands-and-run). Roadmap обновлён.

**Следующий шаг:** Продолжить roadmap-100: блоки 39 (экспорт Otter/Notion), 41 (звуковые уведомления), 62 (SessionCreated), 63 (таймаут D-Bus) или ручная проверка десктопа.
