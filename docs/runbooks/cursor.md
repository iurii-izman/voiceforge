...
Источники правды по порядку: agent-context, next-iteration-focus, PROJECT-STATUS-SUMMARY, planning, plans, audit. При старте сверяйся с board view и бери верхний coherent `Todo` batch, который совпадает с блоком «Следующий шаг» в `next-iteration-focus.md`. Текущий practical queue после закрытия `#114-#122` начинается с `#123`. При начале работы переводи карточку в In Progress, при `Closes #N` — в Done. Не спрашивай пользователя, если ответ можно получить из кода/доков/board/keyring.
