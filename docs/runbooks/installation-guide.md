...
(имя контейнера смотрите в `toolbox list`; на Fedora 43 обычно `fedora-toolbox-43`). После входа приглашение может измениться (например, показывается имя контейнера) — дальше все команды вводите **в этом же терминале (в toolbox)**.
