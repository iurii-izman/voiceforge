...
- **Issues:** открытый внешний риск по-прежнему **#65** (CVE — ждём upstream). В стратегическом queue для движения `76.5 -> 100` уже закрыты **#114** (`main.py` / `server_async.py` hotspot decomposition), **#115** (`llm/router.py` coverage batch), **#116** (`core/daemon.py` behavioral coverage batch), **#117** (`rag/*` lifecycle confidence batch), **#118** (`stt/*` lifecycle/perf confidence batch) и **#119** (`web/API/desktop contract drift prevention batch); открыты **#120-#123**. **#50** (macOS/WSL2) закрыт и снят с активного скоупа.
...
1. **Первый ROI-приоритет:** взять **#120** и закрыть security hardening beyond accepted baseline как следующий code-heavy subsystem batch.
2. **Следом:** взять **#121** (observability proof), затем **#122** (release proof) без смешивания surfaces.
...
- **Новый practical queue:** сначала **#120** (security hardening beyond accepted baseline), затем **#121** (observability proof), затем **#122** (release proof) как три верхних ROI-блока.
...
Задача: взять верхний coherent batch из новой стратегической очереди. Первый приоритет: **#120** — security hardening beyond accepted-risk baseline. Не смешивать этот batch с observability, release или docs/governance surfaces.
```
...
1. **#120**: решить security hardening beyond accepted-risk baseline, не смешивая этот блок с release/manual evidence.
2. **#121**: собрать reproducible Jaeger/runtime evidence, не смешивая это с code-heavy refactor.
3. **#122**: собрать release proof beyond metadata contract, не смешивая это с security/observability.
...
1. Взять **#120** как security hardening batch.
2. Следом взять **#121** как observability evidence batch.
3. Для release confidence отдельно оформить **#122** вокруг metadata/native/updater proof.
...
- **Актуальные strategic items на board:** [#120](https://github.com/iurii-izman/voiceforge/issues/120), [#121](https://github.com/iurii-izman/voiceforge/issues/121), [#122](https://github.com/iurii-izman/voiceforge/issues/122), [#123](https://github.com/iurii-izman/voiceforge/issues/123) плюс внешний риск [#65](https://github.com/iurii-izman/voiceforge/issues/65). [#114](https://github.com/iurii-izman/voiceforge/issues/114), [#115](https://github.com/iurii-izman/voiceforge/issues/115), [#116](https://github.com/iurii-izman/voiceforge/issues/116), [#117](https://github.com/iurii-izman/voiceforge/issues/117), [#118](https://github.com/iurii-izman/voiceforge/issues/118) и [#119](https://github.com/iurii-izman/voiceforge/issues/119) уже закрыты; practical queue теперь начинается с **#120 -> #121 -> #122**.
