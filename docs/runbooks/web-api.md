...
**Ответ (200):** `{ "session_id", "display_text", "analysis" }`

**Ошибки:** 400 (invalid JSON, seconds вне диапазона, неверный template), 422 (pipeline вернул error envelope), 500.

### POST /api/analyze/stream

Доступно только в async server (`voiceforge web --async`). Возвращает SSE-поток с частями LLM-ответа.
...
**Ошибки:** 400 (отсутствуют параметры, не целые числа, нет action items или пустой transcript), 404 (сессия не найдена), 500.

## Формат ошибок

При 4xx/5xx для JSON-ответов тело имеет вид:
...
Коды: `BAD_REQUEST`, `UNAUTHORIZED`, `NOT_FOUND`, `UNPROCESSABLE_ENTITY`, `INTERNAL_ERROR`, `NOT_IMPLEMENTED`, `SERVICE_UNAVAILABLE`.
