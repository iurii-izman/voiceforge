...
| CodeQL alert `py/clear-text-logging-sensitive-data` | GitHub Code Scanning, `scripts/git_credential_keyring_pat.py` | **Dismissed** (QA1 [#152](https://github.com/iurii-izman/voiceforge/issues/152)) | False positive: stdout — Git credential helper protocol; только процесс git читает вывод. В коде комментарий; alert закрыт с rationale в UI.| — |
