...
| CodeQL alert `py/clear-text-logging-sensitive-data` | GitHub Code Scanning, `scripts/git_credential_keyring_pat.py:20`, issue [#152](https://github.com/iurii-izman/voiceforge/issues/152) | `high`, GitHub-visible security debt | Закрыть в QA1 кодом или explicit justified dismissal. До этого считать active remediation item, а не “фон”. | После завершения `#152` |
