...
def test_notify_analyze_done_no_op_without_token() -> None:
    with (
        patch("voiceforge.core.secrets.get_api_key", return_value=None),
        patch("voiceforge.core.telegram_notify.get_telegram_chat_id", return_value="123"),
    ):
        telegram_notify.notify_analyze_done(1, "summary")
    with (
        patch("voiceforge.core.secrets.get_api_key", return_value="token"),
        patch("voiceforge.core.telegram_notify.get_telegram_chat_id", return_value=None),
    ):
        telegram_notify.notify_analyze_done(1, "summary")
...
def test_notify_analyze_done_sends_request_when_configured() -> None:
    ...
    with (
        patch("voiceforge.core.secrets.get_api_key", return_value="fake_token"),
        patch("voiceforge.core.telegram_notify.get_telegram_chat_id", return_value="999"),
        patch("voiceforge.core.telegram_notify.urllib.request.urlopen", side_effect=capture_request),
    ):
