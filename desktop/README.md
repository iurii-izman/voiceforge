...
- если в настройках включено `Скрывать в трей при закрытии окна или Alt+F4`, крестик окна и `Alt+F4` не завершают приложение, а скрывают окно в трей;
- для явного завершения приложения используйте кнопку `Выйти из VoiceForge` в разделе `Настройки → Окно` или пункт `Выход` в трее.
