# VoiceForge — инструкции для Cursor Agent

Краткий контекст (полный — в `docs/runbooks/agent-context.md`). Этот файл подхватывается Cursor автоматически.
...
- **Приоритет фич:** по порядку 1→20 из `docs/plans.md`. Не предлагать фичи вне этого порядка без запроса.
...
- Точечный поиск (grep/codebase_search по символам), без лишних шагов. План развития — сверять с кодом перед реализацией (`docs/development-plan-post-audit-2026.md`, правило plan-verify-before-implement).
